                        ...
                        if (FlushData) // data are handed over for verification/write to disk through FlushBuffer (buffer size DATACON_FLUSHBUFFERSIZE)
                        {
                            ...
                            if (!lowMem)
                            {
                                if (ReadBytesAllocatedSize - ValidBytesInReadBytesBuf > 0)
                                {
                                    ...
                                    {
                                        if (SSL_pending(SSLConn) > 0) // if the internal SSL buffer is not empty, recv() is not called and no further FD_READ arrives, so we must post it ourselves, otherwise the data transfer stops
