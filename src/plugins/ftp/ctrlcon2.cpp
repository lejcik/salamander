        ...
        if (eventError == NO_ERROR)
        {
            if (Socket != INVALID_SOCKET) // socket is connected
            {
                ...
                { // read as many bytes as possible into the buffer; do not read in a loop so the data is processed gradually
                    ...
                    {
                        if (SSL_pending(SSLConn) > 0) // if the internal SSL buffer is not empty, recv() is not called at all and no FD_READ arrives, so post it ourselves to avoid stalling the transfer
